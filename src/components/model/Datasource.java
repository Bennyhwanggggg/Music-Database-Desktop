...
public class Datasource {
    ...
    public boolean open() {
        try {
            ...
            // Statement.RETURN_GENERATED_KEYS to get the returned ids for other usage.
            insertIntoArtists = conn.prepareStatement(INSERT_ARTIST, Statement.RETURN_GENERATED_KEYS);
            insertIntoAlbums = conn.prepareStatement(INSERT_ALBUMS, Statement.RETURN_GENERATED_KEYS);
            insertIntoSongs = conn.prepareStatement(INSERT_SONGS); // We don't need to use the return value from inserting songs.
    ...
    private int insertArtist(String name) throws SQLException {
        ...
        if(results.next()){
            // if already in data base
            return results.getInt(1); // id is at column 1 as set in the prepared statement
        } else {
            // insert artist
            insertIntoArtists.setString(1, name);
            int affectedRows = insertIntoArtists.executeUpdate(); // not using execute as it returns a boolean. executeupdate shows number of rows affected and we want to find out how many we affected since we are only supposed to affect one row.

            if(affectedRows != 1){
                throw new SQLException("Artist insertion failed");
            }
            // retrieve id from newly created id
            ResultSet generatedKeys = insertIntoArtists.getGeneratedKeys();
            if(generatedKeys.next()){
                return generatedKeys.getInt(1);
            } else{
                throw new SQLException("Failed to get _id for artist");
            }
        }
    }
    ...
    private int insertAlbum(String name, int artistId) throws SQLException {
        ...
        if(results.next()){
            ...
            if(affectedRows != 1){
                throw new SQLException("Album insertion failed");
            }
            // retrieve id from newly created id
            ...
            if(generatedKeys.next()){
                ...
                throw new SQLException("Failed to get _id for album");
            }
        }
    }
    ...
    public void insertSong(String title, String artist, String album, int track) {
        try{
            ...
            // cancel transaction when error
            System.out.println("Insert song exception: " + e.getMessage());
            try{
                System.out.println("Changes rollback now...");
                conn.rollback();
